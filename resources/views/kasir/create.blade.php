...
<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
            ...
            <input type="text" wire:model="name" {{-- ✅ wire:model, bukan name= --}}
                class="w-full px-3 py-2 border border-gray-300 rounded-lg
                       focus:ring-2 focus:ring-amber-500 focus:border-amber-500" placeholder="Masukkan nama lengkap kasir">
            @error('name')
        ...
        <div>
            ...
            <input type="email" wire:model="email"
                ...
                placeholder="contoh: kasir@example.com">
            @error('email')
                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
            @enderror
        </div>
    ...
    <div class="flex justify-end gap-4 mt-6">
        <a href="{{ route('kasir.index') }}" wire:navigate
