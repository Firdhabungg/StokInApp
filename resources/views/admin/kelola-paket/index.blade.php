...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-{{ $plans->count() > 3 ? 3 : $plans->count() }} gap-6">
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border {{ $plan->slug === 'pro' ? 'border-amber-200' : 'border-gray-100' }} relative overflow-hidden">
            @if($plan->slug === 'pro')
            ...
            <div class="flex items-center justify-between mb-4">
                <div class="p-3 {{ $plan->slug === 'pro' ? 'bg-amber-50 text-amber-600' : ($plan->slug === 'free' ? 'bg-blue-50 text-blue-600' : 'bg-purple-50 text-purple-600') }} rounded-xl">
                    <i class="fas {{ $plan->slug === 'pro' ? 'fa-bolt' : ($plan->slug === 'free' ? 'fa-gift' : 'fa-crown') }} text-lg"></i>
                </div>
                <span class="text-[10px] font-bold {{ $plan->slug === 'pro' ? 'text-amber-500' : 'text-gray-400' }} uppercase tracking-widest">
            ...
            <table id="paketTable" class="w-full text-sm">
                <thead class="bg-gray-50 text-gray-700 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-center text-[12px] font-bold uppercase tracking-widest">Toko Aktif</th>
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-center">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <form action="{{ route('admin.kelola-paket.toggle', $plan) }}" method="POST" class="inline">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" 
                                        ...
                                        <i class="fas {{ $plan->is_active ? 'fa-toggle-on text-emerald-500' : 'fa-toggle-off' }}"></i>
                                    </button>
                                </form>
                            </div>
                        </td>
