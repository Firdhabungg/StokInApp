...
<aside id="sidebar"
    ...
    <div class="h-20 flex items-center justify-between px-4 flex-shrink-1 border-b border-slate-50">
        ...
        <button onclick="toggleMobileSidebar()" class="md:hidden p-2 text-slate-400">
    ...
    <div class="p-4 border-t border-slate-50">
        ...
        <form method="POST" action="{{ route('logout') }}" id="logout-form">
            ...
            <button type="button" onclick="confirmLogout()"
                class="logout-btn w-full px-4 py-3 bg-rose-600 hover:bg-rose-700 text-white rounded-xl outline-none text-sm font-bold transition-all active:scale-95 shadow-lg flex items-center justify-center gap-2">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Check saved state for Desktop Mini Sidebar
        const isMini = localStorage.getItem('sidebarMini') === 'true';
        if (isMini && window.innerWidth >= 768) {
            sidebar.classList.add('sidebar-mini');
            if (mainWrapper) mainWrapper.classList.replace('md:ml-64', 'md:ml-20');
        }
            ...
            desktopToggle.addEventListener('click', function() {
                ...
                if (mainWrapper) {
                    if (isNowMini) {
                        ...
                        mainWrapper.classList.replace('md:ml-20', 'md:ml-64');
                    }
                }
