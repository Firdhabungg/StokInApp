<header
    ...
    x-data="{ open: false, notifOpen: false, notifications: [], count: 0 }"
    x-init="
        fetch('/notifications/get')
            .then(r => r.json())
            .then(data => { notifications = data.notifications; count = data.count; })
            ...
            <div x-show="notifOpen" 
                ...
                <div class="max-h-64 overflow-y-auto">
                    ...
                    <template x-for="notif in notifications" :key="notif.message">
                        <a :href="notif.link" class="flex items-start gap-3 p-3 hover:bg-gray-50 border-b border-gray-50">
                            <div class="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0"
                                :class="notif.color === 'red' ? 'bg-red-100' : 'bg-orange-100'">
                                <i :class="'fas ' + notif.icon + (notif.color === 'red' ? ' text-red-600' : ' text-orange-600')" class="text-sm"></i>
                            </div>
                ...
                <div class="p-3 border-t bg-gray-50">
                    <a href="{{ route('notifications.index') }}" class="block text-center text-sm text-amber-600 hover:text-amber-700 font-medium">
